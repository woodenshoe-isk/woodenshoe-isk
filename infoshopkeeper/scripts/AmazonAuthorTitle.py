...
handler = logging.handlers.RotatingFileHandler(
    "SQLObject.log", maxBytes=20, backupCount=5
)
...
def main():
    ...
    if amazonQueue.qsize() == 0:
        for x in connection.queryAll(
            connection.sqlrepr(
                sqlbuilder.Select(
                    Title.q.isbn,
                    where=(
                        RLIKE(Title.q.isbn, "^[0-9]{9}[0-9xX]{1}$")
                        & (Title.q.kindID == 1)
                    ),
                    groupBy=Title.q.isbn,
                    limit=100,
                )
            )
        ):
    ...
    while keepRunning:
        try:
            ...
            # otherwise add authors to readline history and print them out
                ...
                inputline = input(
                    "Which set of authors do you want to keep?\n(Separate authors by tabs)\n"
                )
                result = []
                # quit if choice is q
                if inputline == "q":
                ...
                for a in result:
                    ...
                    if len(list(Author.selectBy(authorName=a))) == 0:
                        ...
                        Author.selectBy(authorName=a)[0].authorName.lower() == a.lower()
                    ):
...
class ComparisonThread(Thread):
    ...
    def processTitle(self, item, titleList):
        ...
        corrections = dict(
            bookTitleDirty=False, authorDirty=False, publisherDirty=False
        )
        ...
        if (
            corrections["authorDirty"]
            or corrections["bookTitleDirty"]
            or corrections["publisherDirty"]
        ):
