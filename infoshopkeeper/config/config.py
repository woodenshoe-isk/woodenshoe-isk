        ...
        for root, dirs, files in os.walk('..'):
            for fil in files:
                if fil.endswith('.py'):
                    ...
                    top_packages = [x.split('.')[0] for x in list(mdfinder.modules.keys())]
                    required_modules |= set(top_packages)
    
        dirty=False
        for mod in required_modules:
            if (mod not in builtin_packages) and (mod not in installed_packages) and (mod not in here_packages):
