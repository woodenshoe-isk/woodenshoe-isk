...
_licenseLocations = (
    ...
    (
        lambda key: os.environ.get("AMAZON_LICENSE_KEY", None),
        "an environment variable called AMAZON_LICENSE_KEY",
    ),
    (
        lambda key: _contentsOf(os.getcwd(), _amazonfile1),
        "%s in the current directory" % _amazonfile1,
    ),
    (
        lambda key: _contentsOf(os.getcwd(), _amazonfile2),
        "%s in the current directory" % _amazonfile2,
    ),
    (
        lambda key: _contentsOf(os.environ.get("HOME", ""), _amazonfile1),
        "%s in your home directory" % _amazonfile1,
    ),
    (
        lambda key: _contentsOf(os.environ.get("HOME", ""), _amazonfile2),
        "%s in your home directory" % _amazonfile2,
    ),
    (
        lambda key: _contentsOf(_getScriptDir(), _amazonfile1),
        "%s in the amazon.py directory" % _amazonfile1,
    ),
    (
        lambda key: _contentsOf(_getScriptDir(), _amazonfile2),
        "%s in the amazon.py directory" % _amazonfile2,
    ),
)
_supportedLocales = {
...
def buildURL(
    search_type, keyword, product_line, type, page, license_key, locale, associate
):
    _checkLocaleSupported(locale)
...
def searchByASIN(
    ASIN, type="heavy", license_key=None, http_proxy=None, locale=None, associate=None
):
    return search(
        "AsinSearch", ASIN, None, type, None, license_key, http_proxy, locale, associate
    )


def searchByUPC(
    UPC, type="heavy", license_key=None, http_proxy=None, locale=None, associate=None
):
    return search(
        "UpcSearch", UPC, None, type, None, license_key, http_proxy, locale, associate
    )
...
def searchByManufacturer(
    ...
    if product_line not in (
        ...
        raise AmazonError(
            "product_line must be in ('electronics', 'kitchen', 'videogames', 'software', 'photo', 'pc-hardware')"
        )
    return search(
        "ManufacturerSearch",
        manufacturer,
