...
def searchInventory(sortby='booktitle', out_of_stock=False, **kwargs):
    ...
    if 'publisher' in kwargs:
        where_clause_list.append(RLIKE(Title.q.q.publisher, kwargs['publisher'].strip()))
    if 'tag' in kwargs:
        where_clause_list.append("Title.q.tag RLIKE '%s'" % kwargs['tag'].strip())
    if 'isbn' in kwargs:
    ...
    if 'author' in kwargs:
        where_clause_list.append(RLIKE(author.q.author_name, kwargs['author'].strip()))
    ...
    if 'authorOrTitle' in kwargs:
        where_clause_list.append(OR(RLIKE(Author.q.author_name, kwargs['authorOrTitle'].strip()), RLIKE(Title.q.booktitle, kwargs['authorOrTitle'].strip())))
