...
class SalesReportByOwner(SalesReport):
    ...
    def query(self, args):
        ...
        self.cursor.execute(
            "SELECT * FROM transactionLog WHERE action='SALE' AND info LIKE %s AND date>=%s AND date<=ADDDATE(%s,INTERVAL 1 DAY) and owner=%s order by date",
    ...
    def _queryForm(self):
        ...
        return """<label class='textbox' for='owner'>Owner</label> <select class='textbox' name='owner' id='owner'/>%s</select><br />""" % (
            options
        ) + SalesReport._queryForm(
            self
        )
