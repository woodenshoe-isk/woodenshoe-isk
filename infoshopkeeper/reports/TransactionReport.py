...
class TransactionReport(Report, PdfReport):
    ...
    def query(self, args):
        ...
        if end_date:
            clauses.append(
                "transactionLog.date <= ADDDATE('%s',INTERVAL 1 DAY)" % end_date
            )
    ...
    def format_results(self, results):
        return [
            ...
            % (r.date, r.action, r.info, r.amount)
            for r in results
        ]
    ...
    def _queryForm(self):
        actionList = ",".join(
            [
                "<option value='%s'>%s</option>" % (x[0], x[0])
                for x in Transaction._connection.queryAll(
                    "SELECT DISTINCT t.action FROM transactionLog t ORDER BY t.action"
                )
            ]
        )
        return (
            (
                """<label class='textbox' for='what'>What</label> <input type='text' class='textbox' name='what' id='what' value='%s'/><br>
        <label class='textbox' for='action'>Action</label> <select class='textbox' id='action' name='action'>"""
                + actionList
                + """</select><br>
