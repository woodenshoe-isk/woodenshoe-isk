...
class PdfReport:
    ...
    def makePresentationPage(self, canvas, doc):
        ...
        canvas.drawCentredString(
            self.PAGE_WIDTH / 2.0, self.PAGE_HEIGHT - (0.25 * inch), str(name)
        )
        canvas.line(
            0.5 * inch,
            self.PAGE_HEIGHT - (0.35 * inch),
            self.PAGE_WIDTH - (0.5 * inch),
            self.PAGE_HEIGHT - (0.35 * inch),
        )
        canvas.setFont("Times-Bold", 8)
        canvas.drawCentredString(
            self.PAGE_WIDTH / 2.0, self.PAGE_HEIGHT - (0.45 * inch), str(addr)
        )
        canvas.saveState()
        canvas.setFont("Times-Bold", 16)
        canvas.drawCentredString(
            self.PAGE_WIDTH / 2.0, self.PAGE_HEIGHT - (1 * inch), str(self.reportname)
        )
    ...
    def addPagenum(self, canvas, doc):
        ...
        canvas.drawString(
            0.5 * inch, self.PAGE_HEIGHT - (0.5 * inch), "Page %d" % doc.page
        )
        canvas.drawRightString(
            self.PAGE_WIDTH - (0.5 * inch), self.PAGE_HEIGHT - (0.5 * inch), theDate
        )
        # Footer.
    ...
    def pukePDF(self, filename):
        ...
        doc.build(
            Story, onFirstPage=self.makePresentationPage, onLaterPages=self.addPagenum
        )
