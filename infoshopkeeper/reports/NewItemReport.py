        ...
        self.cursor.execute("""
                SELECT 
                   title.id, title.booktitle, 
                     GROUP_CONCAT(DISTINCT author.author_name), 
                     kind.kind_name, book.listprice 
                FROM book 
                JOIN title
                  ON book.title_id=title.id
                JOIN author_title
                  ON author_title.title_id=title.id
                JOIN author
                  ON author.id = author_title.author_id
                JOIN kind
                  ON kind.id=title.kind_id 
               WHERE kind.id=%s 
               GROUP BY 
