...
class SQLObjectWithFormGlue(SQLObject):
    ...
    def form_to_object(myClass, formdata):
        ...
        try:
            ...
            try:
                ...
                raise NameError(
                    "Class has neither meta.columnList nor _columns attribute"
                )
        for col in cols:
            try:
    ...
    def object_to_form(self, **kwargs):
        def handleForeignKey(col, readOnly=False):
            ...
            colClass = getattr(
                import_module("objects.%s" % colName), colName.capitalize()
            )
            ...
            form_fragment = (
                "<label class='textbox'>%s</label><SELECT name='%sID'class='textbox' %s>"
                % (colName, colName, readOnlyFragment)
            )
            for o in toObjects:
                equals_fragment = ""
                try:
                    ...
                    form_fragment = (
                        ...
                        % (o.id, equals_fragment, getattr(o, colName + "Name"))
                    )  # this is a hack right now and needs to be fixed with a "primary descriptor" member
        ...
        def handleEnum(col, readOnly=False):
            if readOnly:
                ...
                readOnlyFragment = ""
            form_fragment = (
                "<label class='textbox' for='id_%s'>%s</label><select name='%s' class='textbox' %s>"
                % (col.name, col.name, col.name, readOnlyFragment)
            )
            ...
            for enumval in col.enumValues:
                ...
                form_fragment = form_fragment + "<option value='%s' %s>%s</option>" % (
                    enumval,
                    selected_fragment,
                    enumval,
                )
        ...
        formhtml = "<input type='hidden' id='sqlobject_id' name='id' value='%s' />" % (
            self.id
        )
        try:
        ...
        formhtml = (
            formhtml
            + " <div class='button_panel'><input class='reset' type='reset' value='Cancel Changes'><input class='submit' type='submit' value='Save Changes'></div><br />"
        )
        return formhtml
    ...
    def class_to_form(cls, **associatedObjects):
        def handleForeignKey(col):
            ...
            form_fragment = (
                "<label class='textbox'>%s</label><SELECT name='%sID'class='textbox'>"
                % (colName, colName)
            )
            for o in toObjects:
                equals_fragment = ""
        ...
        def handleEnum(col):
            form_fragment = (
                "<label class='textbox' for='id_%s'>%s</label><select name='%s' class='textbox'>"
        ...
        for k, v in list(associatedObjects.items()):
            formhtml = formhtml + "<input type='hidden' name=%s value='%s' /><br>" % (
                k,
                v,
            )
        try:
        ...
        formhtml = (
            formhtml
            + " <input class='reset' type='reset' value='Cancel Changes'><input class='submit' type='submit' value='Save Changes'><br />"
        )
        return formhtml
    ...
    def object_to_view(self):
        def handleForeignKey(col):
            ...
            eval(
                "from objects.%s import %s" % (colName, colName.capitalize()), globals()
            )
