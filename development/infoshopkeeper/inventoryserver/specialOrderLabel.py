...
def print_special_order_label(isbn='', booktitle='', author='', price=0, customer_name='', customer_phone='', customer_email='', num_copies=1):
    ...
    text_object.setXPos(-text_object.getX())
    text_object.textLine('')
    for i in (u'1st', u'2nd', u'3rd'):
            text_object.textLine(u'□ %s call on ___ by _______' % i)
    text_object.textLine('')
    ...
    tmpfile.close()
    print_command_string = string.Template(u"lpr -P $printer -# $num_copies -o media=Custom.175x300 $filename")
