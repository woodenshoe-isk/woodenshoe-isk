...
#                 self._titleedittemplate.title=title
    ...
    @cherrypy.expose
    def check_in_special_order(self, **args):
        ...
        if (args.get('special_orders') and args.get('status')):
            ...
            for so in special_orders:
                so_record=TitleSpecialOrder.get(int(so))
                so_record.orderStatus=args['status']
                specialOrderLabel.print_special_order_label(isbn=so_record.title.isbn, booktitle=so_record.title.booktitle, author=so_record.title.authors_as_string(), price=args.get('price'), customer_name=so_record.specialOrder.customerName, customer_phone=so_record.specialOrder.customerPhoneNumber, customer_email=so_record.specialOrder.customerEmail, num_copies=1)
