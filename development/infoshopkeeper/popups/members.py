...
class AddMemberPanel(wxPanel):
    def __init__(self,parent, main_window, on_successful_add=false, cancel=0):
        ...
        self.add_member_button = wxButton(self, -1, "Add Member")
        EVT_BUTTON(self, self.add_member_button.GetId(), self.OnAdd)

        self.cancel = wxButton(self, -1, "Cancel")
        EVT_BUTTON(self, self.cancel.GetId(), self.OnCancel)
        self.row6.Add(self.add_member_button, 2, wxGROW)
        self.row6.Add(self.cancel, 1, wxGROW)
    ...
    def OnAdd(self,event):
        ...
        if first_name != "" and last_name != "" and phone != "" and len(phone) >= 7 and e_mail != "":
            ...
            self.on_successful_add(id)
            self.statusBar.SetStatusText("%(f)s %(l)s added to members list" % {"f": first_name, "l": last_name})
        else:
            self.statusBar.SetStatusText("Fill all fields !!!")
...
class ShowMembersPanel(wxPanel):
    ...
    def OnSearch(self,event):
        ...
        if len(self.first_nameS) > 0:
            queryTerms["first_name"]=self.first_nameS
        if len(self.last_nameS) > 0:
            queryTerms["last_name"]=self.last_nameS
